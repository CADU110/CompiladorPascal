...
void program(Token *tokens, int *p);
void bloco(Token *tokens, int *p);
void declaracaoVar(Token *tokens, int *p);
void declaracao(Token *tokens, int *p);
int comandoComposto(Token *tokens, int *p);
int comando(Token *tokens, int *p);
int write(Token *tokens, int *p);
int read(Token *tokens, int *p);
int comandoRotulo(Token *tokens, int *p);
int atribuicao(Token *tokens, int *p);
int expressao(Token *tokens, int *p);
int expressaoSimples(Token *tokens, int *p);
int fator(Token *tokens, int *p);
int termo(Token *tokens, int *p);
void variavel(Token *tokens, int *p);
int comandoCondicional(Token *tokens, int *p);
int comandoRepetitivo(Token *tokens, int *p);
void declaracaoRotulos(Token *tokens, int *p);
int desvio(Token *tokens, int *p);
void declaracaoSub(Token *tokens, int *p);
void declaracaoFunction(Token *tokens, int *p);
void declaracaoProcedure(Token *tokens, int *p);
int chamadaProcedure(Token *tokens, int *p);
...
void bloco(Token *tokens, int *p) {
  declaracaoRotulos(tokens,p);
  declaracaoVar(tokens, p);
  declaracaoSub(tokens,p);

  comandoComposto(tokens, p);
...
int comando(Token *tokens, int *p) {
    ...
    return comandoRotulo(tokens, p);
}

int comandoRotulo(Token *tokens, int *p) {
    if (atribuicao(tokens, p)) return 1;
    if (chamadaProcedure(tokens, p)) return 1;
    if (desvio(tokens, p)) return 1;
    if (comandoComposto(tokens, p)) return 1;
    if (comandoCondicional(tokens, p)) return 1;
    if (comandoRepetitivo(tokens, p)) return 1;
    if (write(tokens, p)) return 1;
    if (read(tokens, p)) return 1;
...
int expressao(Token *tokens, int *p) {
    int resultado = expressaoSimples(tokens, p);
    if ((tokens[*p].tipo ==Operador && (strcmp(tokens[*p].token,"=") == 0 || strcmp(tokens[*p].token, "<") == 0 || strcmp(tokens[*p].token, ">") == 0)) || (tokens[*p].tipo == Operador_Composto && (strcmp(tokens[*p].token, "<=") == 0 || strcmp(tokens[*p].token, ">=") == 0 || strcmp(tokens[*p].token, "<>") == 0))){
      (*p)++;
      if(expressaoSimples(tokens, p)==0){
          printf("Rejeito\n");
          exit(0);
      }
    }
...
int fator(Token *tokens, int *p) {
    if (tokens[*p].tipo == Identificador) {
      int p2 = *p;
        while (1) {
            (*p)++;
            expressao(tokens, p); //arrumar aqui
            if (strcmp(tokens[*p].token, ",") == 0) {
                (*p)++;
                int t =expressao(tokens, p);
                if(t==0){
...
void sintia(Token *tokens){
    int currentT=0;

    program(tokens,&currentT);
